    ...
    def xpath(self):
        ...
        if self.name in self.unsupported:
            raise ExpressionError(
                "The pseudo-class %r cannot be supported inline" % self.name)
...
class Pseudo(object):
    ...
    def xpath(self):
        ...
        if self.ident in self.unsupported:
            raise ExpressionError(
                "The pseudo-class %s cannot be supported inline" % self.ident)
        method = '_xpath_' + self.ident.replace('-', '_')
        if not hasattr(self, method):
            raise ExpressionError(
                "The pseudo-class %s is unknown" % self.ident)
        method = getattr(self, method)
