...
DEFAULT_REVIEW_SERVER = "creview.springpartners.com"
...
group.add_option("-c", "--changelist", action="store", dest="changelist", default=None,
                 help=("SVN changelist."))
# Perforce-specific
...
class SubversionVCS(VersionControlSystem):
  ...
  def GenerateDiff(self, args):
    ...
    if self.options.changelist:
      logging.info("Using svn changelist %s" % self.options.changelist)
      cmd += ["--changelist", self.options.changelist]
    cmd.extend(args)
    data = RunShell(cmd)
    count = 0
